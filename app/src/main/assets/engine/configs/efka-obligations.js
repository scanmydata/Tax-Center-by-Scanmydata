/*
 * configs/efka-obligations.js  --  Υποχρεώσεις Ασφάλισης + ΠΒΟ ΚΕΑΟ  (νέο ΟΠΣ e-ΕΦΚΑ)
 *
 * ΓΙΑΤΙ ΔΕΝ ΛΕΓΕΤΑΙ ΠΙΑ `keao-debts.js`: αυτό το config ανήκει στην εφαρμογή και
 * χτυπά **άλλη πύλη** από το ομώνυμο του runner (που είναι η Ηλεκτρονική
 * Πλατφόρμα Οφειλετών ΚΕΑΟ). Με το ίδιο όνομα αρχείου, το `vendor-engine.mjs`
 * θα το αντικαθιστούσε αθόρυβα με το config του runner την επόμενη φορά που θα
 * συγχρόνιζε κάποιος τον engine — και η εφαρμογή θα κατέβαζε άλλα πράγματα από
 * αυτά που λέει η καρτέλα, χωρίς να φανεί πουθενά. Τα δύο συνυπάρχουν επίτηδες:
 * εδώ οι υποχρεώσεις ασφάλισης και τα ΠΒΟ, εκεί η καρτέλα οφειλέτη ανά φορέα.
 *
...
module.exports = {
  ...
  async run(http, inp, lib) {
    ...
    http.log('[efka-obligations] ✅ saved -> ' + path.basename(jf) + ' (' + files.length + ' PDF)');
